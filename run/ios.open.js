...
var OPEN               = "xcrun simctl openurl UUID ";

var cp = require("child_process");
var argv = process.argv.slice(2);

var options = { verbose: true, url: argv[0] };

_getUUID(GREP_BOOTED_DEVICE, function(uuid) {
    if (uuid) {
        cp.exec(OPEN.replace(/UUID/, uuid) + options.url, function(err, stdout, stderr) {
            if (options.verbose) {
        ...
        if (options.verbose) {
            console.log(ERR  + "page open fail. because iOS Simulator shutdown." + CLR);
            console.log(WARN + "please try. `$ npm run ios.start` command." + CLR);
        }
    }
});
