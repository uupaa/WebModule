...
function Module_getDependencies(releaseBuild) { // @arg Boolean = false
                                                // @ret Object - { all:PathStringArray, node:PathStringArray, worker:PathStringArray, browser:PathStringArray, label:StringArray }
        ...
        if (name in packages) {
            ...
            if (config) {
                ...
                if (1) { // [DEPRECATED FORMAT]
                    ...
                    if (target) {
                        console.log(config.name + " is deprecated format. you need do `npm run sync` command");
                    }
                }
...
function Module_collectBuildTarget(json) { // @arg Object - package.json
    ...
    if (wm.all) {
        browserSource = wm.all.source;
        workerSource = wm.all.source;
        nodeSource = wm.all.source;
        allSource = wm.all.source;
        browserOutput = wm.all.output;
        workerOutput = wm.all.output;
        nodeOutput = wm.all.output;
        allOutput = wm.all.output;
    }
    if (wm.browser) {
    ...
    var sources = Module_toUniqueArray([].concat(allSource, browserSource,
                                                 workerSource, nodeSource));
