...
function Module_getDependencies(releaseBuild) { // @arg Boolean = false
    ...
    var files = Module_getFileList(packages, tree, modules, releaseBuild); // { nw, node, worker, browser, label }
...
function Module_collectBuildTarget(json) { // @arg Object - package.json
    ...
    var sources = Module_toUniqueArray([].concat(browserSource, workerSource, nodeSource, nwSource));
    var workDir = "";
