...
function ModuleSystem_getDependencies(releaseBuild) { // @arg Boolean = false
                                                      // @ret Object - { nw:PathStringArray, el:PathStringArray, node:PathStringArray, worker:PathStringArray, browser:PathStringArray, label:StringArray }
    ...
    var files = ModuleSystem_getFileList(packages, tree, modules, releaseBuild); // { nw, el, node, worker, browser, label }
...
function ModuleSystem_getFileList(packages,
                                  tree,
                                  modules,
                                  releaseBuild) { // @ret Object - { nw, el, node, worker, browser, label }
                                                  // @return.nw      PathStringArray - NW.js files.
                                                  // @return.el      PathStringArray - Electron files.
    ...
    var nw = [], el = [], node = [], worker = [], browser = [], label = [];
    ...
    modules.forEach(function(name) {
        if (name in packages) {
            ...
            if (config) {
                ...
                if ("el" in config) {
                    config["el"]["source"].forEach(function(file) {
                        var path = dir + file;
                        if (el.indexOf(path) < 0) { el.push(path); }
                    });
                }
                // --- add Electron support ---
                // package.json に nw があり、el がない場合は、nw の設定を el にコピーして使用する
                // このコードブロックは将来不要になる予定
                if ("nw" in config && !("el" in config)) {
                    config["nw"]["source"].forEach(function(file) {
                        var path = dir + file;
                        if (el.indexOf(path) < 0) { el.push(path); }
                    });
...
function ModuleSystem_collectBuildTarget(json) { // @arg Object - package.json
                                                 // @ret Object - { browser, worker, node, nw, el, sources, workDir }
    ...
    if (wm.el) {
        elSource      = wm.el.source;
        elOutput      = wm.el.output;
    }
    var sources = ModuleSystem_toUniqueArray([].concat(browserSource, workerSource, nodeSource, nwSource, elSource));
    var workDir = "";
    var output = browserOutput || workerOutput || nodeOutput || nwOutput || elOutput || "";
...
//        },
//        "el": {                                           <- add if target.has.node && target.has.browser
//          "source":     ["lib/REPOSITORY_NAME.js"],
...
//  console.log("  upgrade webmodule.target to webmodule{browser|worker|node}");
    ...
    if ( /(all)/i.test(target) || (/(browser)/i.test(target) && /(node)/i.test(target)) ) {
        result.webmodule.el = {
            source: pkg.webmodule.source,
            output: pkg.webmodule.output.replace(/min.js$/, "el.min.js")
        };
    }
    return result;
}
